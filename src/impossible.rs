...
impl FalseData {
    ...
    pub fn new() -> Res<Self> {
        Ok(Self {
            false_type : straight_eq(Zero.ctn()?,Succ(Zero.ctn()?).ctn()?)?.fin()?
        })
    }
    ...
    pub fn exfalso(&self, target: ContainedTerm) -> Res<ContainedTerm> {
        match target.clone().typed()?.pop() {
            Universe(n) => {
                ...
                let exfalso: ContainedTerm = lam_helper_poly([othertype,target,other], self.false_type.get(), "hyp", [n],|[othertype,target,other],hyp,[n]|{
